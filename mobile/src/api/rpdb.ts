...
export interface RPDBWork {
  ...
  faction: string
  armor_type: string
  verification_status: 'unverified' | 'verified' | 'stale' | 'disputed'
...
export function listRPDBWorkRecommendations(id: number, limit = 4) {
  return request.get<{ recommendations: RPDBWork[] }>(`/rpdb/works/${id}/recommendations`, {
    params: { limit },
  })
}
...
export function likeRPDBComment(commentId: number) {
  return request.post<{ active: boolean }>(`/rpdb/comments/${commentId}/like`)
}

export function unlikeRPDBComment(commentId: number) {
  return request.delete<{ active: boolean }>(`/rpdb/comments/${commentId}/like`)
}

export function verifyRPDBWork(id: number, result: 'valid' | 'outdated', comment = '') {
  return request.post<void>(`/rpdb/works/${id}/verify`, { result, comment })
}
...
export function createRPDBList(name: string, description = '', isPublic = false) {
  return request.post<{ list: RPDBList }>('/rpdb/lists', {
    name,
    description,
    is_public: isPublic,
  })
...
export function addRPDBWorkToList(id: number, status: RPDBListStatus = 'wanted', listId?: number) {
  return request.post<void>(`/rpdb/works/${id}/list`, {
    status,
    list_id: listId,
  })
}
...
export function exportRPDBList(listId: number, format: 'json' | 'csv' | 'tomtom') {
  return request.get<{
    ...
    content?: string
    list?: RPDBList
    missing_coordinates?: Array<{ work_id: number; title: string }>
  }>(`/rpdb/lists/${listId}/export`, { params: { format } })
}
