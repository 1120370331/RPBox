...
test('fresh and recovery states are strict and linkage uses version state plus one item', () => {
  ...
  assert.match(linkageWait, /resolve_submission_items_version_id\(items, included\) != version_id/)
...
test('item linkage resolves exactly through collection include without any item self GET', () => {
  assert.match(workflow, /def get_collection_with_included/)
  ...
  assert.match(workflow, /def resolve_submission_items_version_id/)
  assert.ok(workflow.includes('f"/v1/reviewSubmissions/{submission_id}/items"'))
  assert.match(workflow, /params=\{"include": "appStoreVersion", "limit": 50\}/)
  ...
  assert.doesNotMatch(workflow, /\/v1\/reviewSubmissionItems\/\{[^}]+\}/)
  assert.doesNotMatch(
    workflow,
    /\/v1\/reviewSubmissionItems\/\{[^}]+\}\/appStoreVersion/,
  )
