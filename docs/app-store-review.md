...
| 公开隐私政策 URL | BLOCKED | 2026-08-24 匿名请求 `https://totalrpbox.com/privacy.html` 返回 HTTP 200，但线上内容尚不是本次候选的完整双语政策；仓库源文件为 `shared/privacy.html` | 部署最终版本；用未登录浏览器和移动网络确认 HTTPS、HTTP 200、无重定向死循环、中文和英文完整可读；再填入 App Store Connect |
...
| iOS 签名、配置与更新 URL | BLOCKED | GitHub 已配置 Team、Bundle、证书、profile 和 App Store Connect API 密钥，历史流水线曾成功签名上传；iOS 准备与 verifier 共用 `mobile/scripts/iosCompliance.mjs` 的权限文案，公开更新 URL 只接受真实 `apps.apple.com` 规范路径 | 对 2.0.2 候选完成 archive、导出、ASC processing 与 TestFlight 安装；TestFlight 上传不得提前更新生产 updater，凭据不写入仓库 |
| 稳定性、后端与演示账号（2.1） | BLOCKED | 2026-08-24 验证当前生产地址 `https://ksxvodevhonx.sealosbja.site`：DNS/TLS 正常，`/health` 返回 200，数据库驱动的 `/api/v1/rpdb/works` 返回 200，匿名访问受保护路径返回 401。`rpbox.app` 本身未注册，因此不得将 `api.rpbox.app` 编入候选 | 保持现有生产主机在线，并从第二个独立网络复核；创建长期有效的审核账号和单独的一次性删除账号，在 App Store Connect Review Information 安全填写，再用 TestFlight/真机验证真实登录和代表性流程 |
| 截图、描述与版本元数据（2.3） | BLOCKED | Apple 公开记录仍为 1.0、4+，描述较旧；App Privacy 当前还声明了音频、游戏内容及多类诊断数据，需与最终数据流逐项核对。本地已有 1242×2208 与 1242×2688 的候选截图，但尚未证明已上传或与最终 IPA 一致 | 创建 2.0.2 商店版本并选择已处理 build；复核截图权利与最终 UI，更新本地化描述、隐私/支持 URL、App Privacy、年龄分级和审核备注；不展示尚未上线功能 |
...
| 年龄分级、UGC 与知识产权 | BLOCKED | 条款已发布 UGC 禁止行为和 Blizzard/WoW 非隶属声明，但 App Store Connect 问卷和素材权利无法由仓库证明 | 如实申报 UGC、聊天/评论能力和实际内容强度；核查截图、图标、名称、示例人物卡及用户内容的使用权；避免暗示 Blizzard 赞助或认可 |
