...
| iOS 签名、配置与更新 URL | BLOCKED | GitHub 已配置 Team、Bundle、证书、profile 和 App Store Connect API 密钥，历史流水线曾成功签名上传；ASC 已存在待提交版本 1.1，最新有效 TestFlight build 为 1.0.40（1000040）。iOS 准备与 verifier 共用合规文案，公开更新 URL 强制绑定真实 App ID 6761112311 | 对 1.1 / 1000041 候选完成 archive、导出、ASC processing 与 TestFlight 安装；TestFlight 上传不得提前更新生产 updater，凭据不写入仓库 |
...
| 截图、描述与版本元数据（2.3） | BLOCKED | ASC 已有 1.1 `PREPARE_FOR_SUBMISSION` 记录、1 个中文本地化和 4 张截图；描述、关键词、支持/营销 URL 与审核联系人存在，但尚无已选 build、What’s New 或审核备注。应用仍支持 iPad，而 1.1 审计只返回 1 个截图尺寸组。公开 1.0 的 App Privacy 还声明音频、游戏内容及多类诊断数据 | 选择已处理的 1.1 build；复核并补齐 iPad 截图或确认继承规则，更新 What’s New、审核备注、隐私/支持 URL 和 App Privacy；截图须与最终 UI 一致且不展示未上线功能 |
