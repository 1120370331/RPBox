...
import { deleteRPDBWork, listMyRPDBWorks, resolveRPDBMediaURL, updateRPDBWorkVisibility, type RPDBVisibility, type RPDBWork } from '@/api/rpdb'
...
const counts = computed(() => ({
  ...
  draft: works.value.filter(work => work.status === 'draft').length,
...
async function load() {
  loading.value = true
  try {
    const [workResult, guildResult] = await Promise.all([listMyRPDBWorks(), listGuilds()])
...
<template>
  <main class="uploads-page">
    ...
    <div v-else-if="!filteredWorks.length" class="page-state empty">
      <i class="ri-upload-cloud-2-line"></i>
      <h2>这个分类还没有内容</h2>
      <button type="button" @click="router.push('/rpdb/create')">发布第一份作品</button>
    </div>
    <section v-else class="uploads-list" data-testid="rpdb-my-uploads-list">
