...
import { listRPDBWorks, resolveRPDBMediaURL, type ListRPDBWorksParams, type RPDBWork, type RPDBWorkType } from '@/api/rpdb'
...
const featuredSlots = computed(() => Array.from({ length: 3 }, (_, index) => works.value[index]))
...
<template>
  <div class="rpdb-shell">
    <main class="discovery-main">
      ...
      <section class="featured-grid" aria-label="编辑精选">
        <button
          v-for="(work, index) in featuredSlots"
          :key="work?.id || `featured-empty-${index}`"
          type="button"
          class="featured-card"
          :class="{ empty: !work }"
          :disabled="!work"
        ...
        >
          ...
          <div class="featured-copy">
            <span>{{ work ? typeLabel(work.type) : ['等待物品精选', '等待幻化精选', '等待家宅精选'][index] }}</span>
            <h2>{{ work?.title || '社区精选席位' }}</h2>
            <p>{{ work?.summary || '优质玩家作品将在审核后展示于此。' }}</p>
            <div class="featured-meta">
              <b>{{ work?.author_name || 'RPBox 编辑部' }}</b>
              <div v-if="work" class="featured-metrics" aria-label="作品数据" data-testid="rpdb-featured-metrics">
                <span title="浏览"><i class="ri-eye-line"></i>{{ formatCount(work.view_count) }}</span>
      ...
      <section class="feed-heading">
        ...
        <div class="view-mode"><i class="ri-layout-grid-fill"></i><span v-if="total">显示 {{ pageStart }}-{{ pageEnd }}</span><span v-else>媒体视图</span></div>
      </section>
      ...
      <div v-else-if="works.length" class="discovery-grid"><RPDBWorkCard v-for="work in works" :key="work.id" :work="work" @open="openWork(work)" /></div>
...
@media(max-width:1180px){.content-nav{align-items:flex-start;flex-direction:column}.featured-grid{grid-template-columns:1fr 1fr}.featured-card:last-child{display:none}.discovery-main{padding:18px}}
@media(max-width:820px){.content-nav{overflow:hidden}.content-nav__channels,.content-nav__actions{width:100%;overflow-x:auto}.featured-grid{grid-template-columns:1fr}.featured-card{min-height:260px}.featured-card:nth-child(n+2){display:none}.channel-tabs{overflow:auto}.discovery-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:520px){.discovery-main{padding:12px}.content-nav__actions{display:none}.featured-card{min-height:220px}.featured-copy{inset:auto 18px 16px}.featured-copy h2{font-size:21px}.discovery-grid{grid-template-columns:1fr}.view-mode{display:none}}
</style>
