export default {
  title: 'Community',
  pageTitle: 'Tavern Board',
  pageSubtitle: '"Stories and quests from across Azeroth gather here..."',
  tabs: {
    all: 'All',
    hot: 'Hot',
    latest: 'Latest',
    following: 'Following',
  },
  feed: {
    posts: 'Posts',
    events: 'Events',
  },
  banner: {
    kicker: 'Tonight\'s Board',
    count: '{count} live / upcoming',
    when: 'When',
    where: 'Where',
    viewDetail: 'View Event',
    viewAll: 'All Events',
    prev: 'Previous event',
    next: 'Next event',
    emptyTitle: 'No live events yet',
    emptyBody: 'Publish an event to feature it on the community banner.',
    emptyAction: 'Create Event',
  },
  eventStatus: {
    live: 'Live',
    upcoming: 'Upcoming',
    ended: 'Ended',
  },
  eventFilter: {
    active: 'Active',
    all: 'All Status',
  },
  countdown: {
    daysHours: '{days}d {hours}h',
    hoursMinutes: '{hours}h {minutes}m',
    minutes: '{minutes}m',
    startsIn: 'Starts in {time}',
    endsIn: 'Ends in {time}',
  },
  post: {
    title: 'Title',
    content: 'Content',
    author: 'Author',
    createTime: 'Posted',
    views: 'Views',
    likes: 'Likes',
    comments: 'Comments',
    shares: 'Shares',
    featured: 'Featured',
  },
  action: {
    create: 'New Post',
    edit: 'Edit',
    delete: 'Delete',
    reply: 'Reply',
    like: 'Like',
    unlike: 'Unlike',
    share: 'Share',
    report: 'Report',
    pin: 'Pin',
    unpin: 'Unpin',
    publish: 'Publish',
    favorites: 'Favorites',
    history: 'History',
    myPosts: 'My Posts',
  },
  comment: {
    placeholder: 'Write a comment...',
    submit: 'Submit',
    reply: 'Reply',
    delete: 'Delete',
    noComments: 'No comments yet',
  },
  empty: 'No posts yet',
  emptyAction: 'Create the first post',
  loading: 'Loading...',
  regionSearchPlaceholder: 'Search region, e.g. Goldshire...',
  addressSearchPlaceholder: 'Search address, e.g. Lion\'s Pride Inn...',
  filter: {
    search: 'Search titles, content, authors, regions, or places...',
    authorSearch: 'Search by author...',
    category: 'Category',
    all: 'All',
    sortLabel: 'Sort:',
    sortLatest: 'Latest',
    sortHot: 'Popular',
    sortViews: 'Most Viewed',
  },
  guild: {
    filterBanner: 'Viewing content from "{name}"',
    clearFilter: 'Clear Filter',
  },
  pinned: {
    title: 'Pinned',
    tag: 'Pinned',
  },
  events: {
    title: 'Upcoming Events',
    empty: 'No events match the current filters',
    calendarTitle: 'Event Calendar',
    synced: 'Synced',
    today: 'Today',
    clickToView: 'Click an event to view details.',
    switchToList: 'Switch to list view',
    switchToCalendar: 'Switch to calendar view',
    collapse: 'Collapse',
  },
  calendar: {
    monthEvents: 'This Month',
    coverDays: 'Across {days} days',
    peakDate: 'Peak Date',
    mainType: 'Main Type',
    sessions: '{count} events',
    typeFilter: 'Type Filter',
    dayCount: '{count}',
  },
  eventType: {
    all: 'All',
    server: 'Server Event',
    guild: 'Guild Event',
    other: 'Event',
  },
  weekDays: {
    sun: 'Sun',
    mon: 'Mon',
    tue: 'Tue',
    wed: 'Wed',
    thu: 'Thu',
    fri: 'Fri',
    sat: 'Sat',
  },
  time: {
    justNow: 'Just now',
    hoursAgo: '{hours}h ago',
    daysAgo: '{days}d ago',
  },
  pagination: {
    prev: 'Previous',
    next: 'Next',
    pageInfo: 'Page {current} / {total}',
  },
  category: {
    other: 'Other',
  },
  create: {
    pageTitle: 'Create New Scroll',
    titlePlaceholder: 'Enter an engaging title...',
    contentPlaceholder: 'Begin writing your legend...',
    coverLabel: 'Cover Image (Optional)',
    coverProcessing: 'Processing...',
    coverUpload: 'Click to upload cover image',
    coverHint: 'Recommended 16:9 ratio, auto-compressed to 1MB',
    coverPreview: 'Cover Preview',
    coverUploadSuccess: 'Cover image uploaded',
    coverUploadFailed: 'Cover upload failed',
    imageProcessFailed: 'Image processing failed',
    noImageUrl: 'Failed to get image URL',
    selectImageFile: 'Please select an image file',
    category: 'Category',
    eventType: 'Event Type',
    eventTypeServer: 'Server',
    eventTypeGuild: 'Guild',
    tags: 'Tags',
    guild: 'Guild',
    guildNone: 'None',
    guildSelect: 'Select',
    region: 'Region',
    regionPlaceholder: 'e.g. Goldshire, Boralus',
    address: 'Address',
    addressPlaceholder: 'e.g. Lion\'s Pride Inn, The Curious Octopus',
    visibility: 'Visibility',
    visibilityPublic: 'Visible to everyone',
    visibilityPrivate: 'Only visible to yourself',
    visibilityGuildPublic: 'Post will also appear in community',
    visibilityGuildPrivate: 'Guild members only',
    eventTime: 'Event Time',
    eventStartTime: 'Start Time',
    eventEndTime: 'End Time',
    eventColor: 'Event Marker Color',
    eventTypeRequired: 'Please choose an event type',
    eventStartRequired: 'Please set the event start time',
    eventCalendarGuideTitle: 'Event Banner Linking',
    eventCalendarGuideBody: 'Choose event type and set the start time. After approval it appears on the community banner and events feed.',
    eventCalendarGuideGuild: 'If you choose "Guild", please select the guild first.',
    quickJump: 'Add Internal Link',
    preview: 'Preview',
    cancel: 'Cancel',
    publish: 'Publish',
    saveDraft: 'Save Draft',
    publishSuccess: 'Published, pending review. Events appear on the banner after approval',
    draftSuccess: 'Draft saved',
    submitFailed: 'Submit failed, please retry',
    loginRequired: 'Please login first',
    titleRequired: 'Please enter a title',
    contentRequired: 'Please enter content',
    noServerEventPermission: 'No permission to post server events',
    noGuildEventPermission: 'No permission to post guild events',
    selectGuildForEvent: 'Please select a guild for the event',
  },
  drafts: {
    title: 'Drafts',
    count: '{count} cloud drafts',
    new: 'New draft',
    loading: 'Loading drafts...',
    empty: 'No drafts yet',
    untitled: 'Untitled draft',
    delete: 'Delete draft',
    deleteTitle: 'Delete Draft',
    deleteMessage: 'Delete "{title}"? This action cannot be undone.',
    idle: 'Cloud autosave',
    saving: 'Saving...',
    saved: 'Saved to cloud',
    error: 'Cloud save failed',
    resumeTitle: 'Resume unpublished draft?',
    resumeMessage: 'An unpublished draft was found. Continue to edit it, or discard and start blank.',
    resumeContinue: 'Use local version',
    resumeDiscard: 'Start blank',
    useCloud: 'Use cloud version',
    localNewerMessage: 'A newer local draft snapshot was found. Apply it over the cloud content?',
    invalidDraft: 'This draft is missing or already published',
    cannotDraftPublished: 'Published posts cannot be reverted to drafts',
  },
  edit: {
    pageTitle: 'Edit Scroll',
    loading: 'Loading...',
    postNotFound: 'Post not found',
    updateSuccess: 'Updated successfully',
    updateFailed: 'Update failed, please retry',
    deleteTitle: 'Delete Post',
    deleteMessage: 'Are you sure you want to delete this post? This action cannot be undone.',
    deleteSuccess: 'Post deleted',
    deleteFailed: 'Delete failed, please retry',
    save: 'Save',
    delete: 'Delete',
    loginRequired: 'Please login first to edit posts',
  },
  detail: {
    back: 'Back',
    viewImage: 'View Image',
    location: 'Location',
    invalidPostId: 'Invalid post ID',
    loadFailed: 'Failed to load post',
    discussion: 'Discussion',
    emptyComments: 'No comments yet, be the first to comment',
    commentPlaceholder: 'Share your thoughts...',
    postComment: 'Post Comment',
    commentFailed: 'Comment failed',
    deleteCommentTitle: 'Delete Comment',
    deleteCommentMessage: 'Are you sure you want to delete this comment?',
    deleteCommentFailed: 'Delete failed: ',
    likeFailed: 'Like failed',
    replyTo: 'Reply to {name}',
    replyToLabel: 'Reply to',
  },
  preview: {
    backToEdit: 'Back to Edit',
    previewMode: 'Preview Mode',
    unknownUser: 'Unknown User',
    noTitle: 'No Title',
    noContent: 'No Content',
    commentsAfterPublish: 'Comments will appear after publishing',
  },
  myPosts: {
    pageTitle: 'My Posts',
    back: 'Back',
    createPost: 'Create Post',
    createFirstPost: 'Create Your First Post',
    published: 'Published',
    pending: 'Pending',
    draft: 'Draft',
    draftBox: 'Drafts',
    myPublished: 'My Published',
    searchPlaceholder: 'Search title or content...',
    allCategories: 'All Categories',
    noMatchingPosts: 'No matching posts',
    noDrafts: 'No drafts',
    noPendingPosts: 'No pending posts',
    noPublishedPosts: 'No published posts',
    deleteTitle: 'Delete Post',
    deleteMessage: 'Are you sure you want to delete "{title}"? This action cannot be undone.',
    deleteSuccess: 'Post deleted',
    deleteFailed: 'Delete failed, please retry',
    setPublic: 'Set Public',
    setPrivate: 'Set Private',
    visibilityChanged: 'Visibility updated',
    visibilityChangeFailed: 'Failed to update visibility',
  },
}
