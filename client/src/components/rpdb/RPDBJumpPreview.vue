...
const previewTypes: Record<RPDBWorkType, { label: string; icon: string; variant: string }> = {
  item_showcase: { label: '魔兽物品', icon: 'ri-magic-line', variant: 'item' },
  transmog: { label: '幻化方案', icon: 'ri-shirt-line', variant: 'transmog' },
  home_showcase: { label: '家宅分享', icon: 'ri-home-heart-line', variant: 'home' },
}
...
const title = computed(() => work.value?.title || fallback.value?.title || '未命名作品')
const summary = computed(() => work.value?.summary || work.value?.effect_description || fallback.value?.summary || '作者尚未填写作品摘要。')
const author = computed(() => work.value?.author_name || fallback.value?.author || '匿名贡献者')
...
const metrics = computed(() => [
  { icon: 'ri-eye-line', title: '浏览', value: work.value?.view_count ?? fallback.value?.views ?? 0 },
  { icon: 'ri-heart-3-line', title: '点赞', value: work.value?.like_count ?? fallback.value?.likes ?? 0 },
  { icon: 'ri-bookmark-3-line', title: '收藏', value: work.value?.favorite_count ?? fallback.value?.favorites ?? 0 },
  { icon: 'ri-list-check-3', title: '加入清单', value: work.value?.list_count ?? fallback.value?.lists ?? 0 },
])
...
function handleFocusOut(event: FocusEvent) {
  if (findCard(event.target) === activeCard) closePreview()
}
    ...
    <Transition name="rpdb-jump-preview">
      ...
      >
        ...
        <div v-else class="rpdb-jump-preview__body" :class="{ loading }">
          ...
          <h3>{{ title }}</h3>
          <p>{{ loading ? '正在加载最新预览...' : summary }}</p>
          <div class="rpdb-jump-preview__metrics" aria-label="作品数据">
