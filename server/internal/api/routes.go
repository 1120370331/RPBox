...
func (s *Server) setupRoutes() {
	...
	v1.Use(middleware.RateLimit(s.cfg.RateLimit.API.RPS, s.cfg.RateLimit.API.Burst))
	{
		...
		auth.Use(middleware.JWTAuth())
		{
			...
			auth.GET("/rpdb/drafts", s.listRPDBDrafts)
			auth.POST("/rpdb/drafts", s.createRPDBDraft)
			auth.GET("/rpdb/drafts/:id", s.getRPDBDraft)
			auth.PUT("/rpdb/drafts/:id", s.updateRPDBDraft)
			auth.DELETE("/rpdb/drafts/:id", s.deleteRPDBDraft)
			auth.POST("/rpdb/drafts/:id/publish", s.publishRPDBDraft)
