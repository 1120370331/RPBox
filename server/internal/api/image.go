...
// migrateLegacyItemPreviewIfNeeded migrates legacy base64 item preview values to uploaded URLs.
func (s *Server) migrateLegacyItemPreviewIfNeeded(c *gin.Context, id string, raw string) string {
	value := strings.TrimSpace(raw)
	if value == "" || isImageURL(value) {
		return value
	}

	normalized, err := s.normalizeAndStoreImageValue(c, value, fmt.Sprintf("items/%s/preview", id))
	if err != nil {
		return value
	}
	normalized = strings.TrimSpace(normalized)
	...
	now := time.Now()
	_ = database.DB.Model(&model.Item{}).Where("id = ?", idNum).Updates(map[string]interface{}{
